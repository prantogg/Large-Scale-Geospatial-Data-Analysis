...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val countDF = spark.sql("SELECT rectangle, COUNT(point) AS count FROM joinResult GROUP BY rectangle ORDER BY rectangle ASC")
    countDF.show()
    return countDF // YOU NEED TO CHANGE THIS PART
  }
